...
exports.likeRoom = asyncHandler(async (req, res) => {
  const room = await roomService.handleReaction(req.room ?? await roomService.getRoomById(req.params.id), req.user.userId, 'like');
  return res.json({ success: true, message: 'Room liked!', room });
});

exports.dislikeRoom = asyncHandler(async (req, res) => {
  const room = await roomService.handleReaction(req.room ?? await roomService.getRoomById(req.params.id), req.user.userId, 'dislike');
